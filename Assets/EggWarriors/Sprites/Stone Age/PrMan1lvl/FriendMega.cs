...
public class FriendMegaAttack: MonoBehaviour
{
    ...
    public void Attack()
    {
        ...
        if (_anim.GetFloat("RunTG") == 1)
        {
            ...
            for (int i = 0; i < enemiesToDamage.Length; i++) 
            {
                ...
                try
                {
                    enemiesToDamage[i].GetComponent<Enemy>().TakeDamage(damagef);
                    
                }
                catch
                {
                    enemiesToDamage[i].GetComponent<EnemyBase>().TakeDamage(damagef);
